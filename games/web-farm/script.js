        ...
        let CROP_DATA = {
            ...
            'chilli': { name: 'Chilli', icon: '<img src="https://raw.githubusercontent.com/guahhinc/webfarm/main/chilli.png" alt="Chilli" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: 2500, sellPrice: 5200, growTime: 110000, weightRange: [0.8, 1.25], rarity: 'rare' },
            ...
            'litchi': { name: 'Litchi', icon: '<img src="http://raw.githubusercontent.com/guahhinc/webfarm/main/litchi.png" alt="Litchi" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: 15000000, sellPrice: 19000000, growTime: 500000, weightRange: [0.9, 1.1], rarity: 'ultra_rare', stockChance: 0.1 },
            'fig': { name: 'Fig', icon: '<img src="http://raw.githubusercontent.com/guahhinc/webfarm/main/fig.png" alt="Fig" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: 50000000, sellPrice: 31000000, growTime: 520000, weightRange: [0.9, 1.1], rarity: 'ultra_rare', stockChance: 0.08 },
        ...
        const buyGear = (key, qty = 1) => {
            ...
            const stockLeft = gameState.shopGearStock?.[key] ?? Infinity;
            ...
            if (gameState.shopGearStock && Number.isFinite(gameState.shopGearStock[key])) {
                gameState.shopGearStock[key] -= qty;
            ...
            if (buyGearsEl) {
                ...
                buyGearsEl.innerHTML = allGearsKeys.map(k => {
                    ...
                    if (!isInStock) {
                        ...
                        const stockRemaining = gameState.shopGearStock?.[k] ?? Infinity;
                        const stockLabel = Number.isFinite(stockRemaining) ? `x${stockRemaining}` : '';
                        const totalCost = g.cost;
                        const isDisabledClass = gameState.money < totalCost ? 'disabled' : '';
                        const titleText = `Buy ${g.name} for ${formatMoney(totalCost)} 💰 - ${g.description}${stockLabel ? ` (Stock: ${stockRemaining})` : ''}`;
                        return `<li class="plant-button ${isDisabledClass}" onclick="buyGear('${k}', 1)" title="${titleText}"><span class="shop-tile-icon">${g.icon}</span><span class="shop-tile-name">${g.name}</span>${stockLabel ? `<span class="shop-tile-stock">${stockLabel}</span>` : ''}<span class="shop-tile-price">${formatMoney(totalCost)}💰</span></li>`;
                    }
                }).join('');
            }
        ...
        const loadGame = (rawSaveData) => {
            ...
            try {
                ...
                newState.usedPromoCodes = Array.isArray(dataToProcess.usedPromoCodes) ? dataToProcess.usedPromoCodes.slice() : [];
        ...
        const PROMO_REWARDS = {
            'TACO': (state) => {
                const baseInfo = CROP_DATA['taco'];
                if (!baseInfo) return;
                if (!state.inventory.produce['taco']) {
                    state.inventory.produce['taco'] = { count: 0, totalWeight: 0.0, isFavorited: false };
                }
                const range = baseInfo.weightRange || [1.0, 1.0];
                const avgWeight = (range[0] + range[1]) / 2;
                state.inventory.produce['taco'].count += 1;
                state.inventory.produce['taco'].totalWeight += avgWeight;
                addMessage("Promo 'TACO' redeemed: +1 Taco!", 'success', true);
            },
            'MONEYPLS': (state) => {
                state.money += 20000;
                addMessage("Promo 'MONEYPLS' redeemed: +20,000💰!", 'success', true);
            },
            '67': (state) => {
                state.money += 67000;
                addMessage("Promo '67' redeemed: +67,000💰!", 'success', true);
            },
        };
        ...
        const redeemPromoCode = () => {
            ...
            if (!raw) {
                addMessage("Enter a promo code first.", 'error', true);
                return;
            }
            const code = raw.toUpperCase();
            const handler = PROMO_REWARDS[code];
            if (!handler) {
                addMessage("Unknown or expired promo code.", 'error', true);
                return;
            }
            if (!Array.isArray(gameState.usedPromoCodes)) gameState.usedPromoCodes = [];
            if (gameState.usedPromoCodes.includes(code)) {
                addMessage("You have already used this promo code.", 'error', true);
                return;
            }
            handler(gameState);

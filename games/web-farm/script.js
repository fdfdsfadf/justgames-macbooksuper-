        ...
        let CROP_DATA = {
            ...
            'dragonfruit': { name: 'Dragonfruit', icon: '<img src="https://raw.githubusercontent.com/guahhinc/webfarm/main/dragonfruit.png" alt="Dragonfruit" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: 1000000, sellPrice: 1500000, growTime: 400000, weightRange: [0.95, 1.05], rarity: 'ultra_rare', stockChance: 0.08 },
            'apple': { name: 'Apple', icon: '<img src="https://raw.githubusercontent.com/guahhinc/webfarm/main/apple.png" alt="Apple" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: null, sellPrice: 110, growTime: null, weightRange: [0.9, 1.1] },
            'banana': { name: 'Banana', icon: '<img src="https://raw.githubusercontent.com/guahhinc/webfarm/main/banana.png" alt="Banana" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: null, sellPrice: 180, growTime: null, weightRange: [0.85, 1.15] },
            'orange': { name: 'Orange', icon: '<img src="https://raw.githubusercontent.com/guahhinc/webfarm/main/orange.png" alt="Orange" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: null, sellPrice: 145, growTime: null, weightRange: [0.9, 1.1] },
            'pineapple': { name: 'Pineapple', icon: '<img src="https://raw.githubusercontent.com/guahhinc/webfarm/main/pineapple.png" alt="Pineapple" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: 20000, sellPrice: 30000, growTime: 48000, weightRange: [0.8, 1.2] },
            'coconut': { name: 'Coconut', icon: '<img src="https://raw.githubusercontent.com/guahhinc/webfarm/main/coconut.png" alt="Coconut" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: null, sellPrice: 2400, growTime: null, weightRange: [0.9, 1.3] },
        };
        ...
        const MAX_REBIRTHS = 15;
        const REBIRTH_BASE_COST = 75000;
        const REBIRTH_COST_SCALE = 3.5;
        const REBIRTH_MULTIPLIER_PER_LEVEL = 0.076;
        ...
        const sellProduce = (key, qty = 1) => {
            ...
            if (itemData?.isFavorited) {
                addMessage(`Cannot sell a 🔒 locked item. Right-click to unlock.`, 'error');
                return false;
            }
        ...
        const toggleLockProduce = (itemKey) => {
            if (gameState && gameState.inventory.produce[itemKey]) {
                ...
                const { baseKey, statusKeys } = parseItemKey(itemKey);
                const baseInfo = CROP_DATA[baseKey];
                // Check if this is a rare item that needs confirmation before unlocking
                if (item.isFavorited) {
                    const isRare = (baseInfo?.rarity === 'ultra_rare') ||
                        (baseInfo?.seedCost !== null && baseInfo?.seedCost >= 35000) ||
                        statusKeys.some(s => ['cosmic', 'rainbow'].includes(s));
                    if (isRare) {
                        const displayName = (statusKeys.length > 0 ? statusKeys.map(s => SPECIAL_STATUSES[s]?.name).filter(Boolean).join(' ') + ' ' : '') + (baseInfo?.name || baseKey);
                        if (!confirm(`Unlock ${displayName}? This is a rare item and will be sellable again.`)) return;
                    }
                }
        ...
        const updateUI = () => {
            ...
            buySeedsEl.innerHTML = plantableCropsOverall.map(k => {
                ...
                if (!isInStock) {
                    ...
                    const titleText = co === 0 ? "Get for Free" : `Buy ${purchaseQty} ${c.name} for ${totalCost.toLocaleString()}💰`;
                    const isDisabledClass = (co > 0 && gameState.money < totalCost) ? 'disabled' : '';
                    return `<li class="plant-button ${isDisabledClass}" onclick="buySeeds('${k}',${purchaseQty})" title="${titleText}"><span class="shop-tile-icon">${c.icon}</span><span class="shop-tile-name">${c.name}</span><span class="shop-tile-price">${costText}</span></li>`;
                }
            }).join('');
            ...
            if (buyGearsEl) {
                ...
                buyGearsEl.innerHTML = allGearsKeys.map(k => {
                    ...
                    if (!isInStock) {
                        ...
                        const totalCost = g.cost * purchaseQty;
                        const isDisabledClass = gameState.money < totalCost ? 'disabled' : '';
                        const titleText = `Buy ${purchaseQty} ${g.name} for ${formatMoney(totalCost)} 💰 - ${g.description}`;
                        return `<li class="plant-button ${isDisabledClass}" onclick="buyGear('${k}', ${purchaseQty})" title="${titleText}"><span class="shop-tile-icon">${g.icon}</span><span class="shop-tile-name">${g.name}</span><span class="shop-tile-price">${formatMoney(totalCost)}💰</span></li>`;
                    }
                }).join('');
            }
            ...
            if (rebirthContentEl) {
                ...
                if (atMax) {
                    ...
                    rebirthHTML += `<button onclick="performRebirth()" ${canRebirth ? '' : 'disabled'} title="Spend ${cost.toLocaleString()}💰 to reset farm for +15% sell multiplier and +1 starting plot">\ud83d\udd04 Rebirth (Next: ${nextMulti.toFixed(2)}x)</button>`;
                }
